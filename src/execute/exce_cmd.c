...
/*   Updated: 2025/02/02 00:23:36 by dasalaza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int execute_commands(t_mini *mini)
{
    ...
    while (t_list_exec_cmd)
    {
        ...
        // if (has_been_redirection_applied == FALSE)
        // {
        // t_list_exec_cmd = t_list_exec_cmd->next;
        // continue;
        // }
  ...
  //   	char message[256];
		// snprintf(message, sizeof(message), "PID padre: [%d]\n", getpid());
  //   	perror(message);
        pid = fork();
        ...
        if (pid == 0)
        {
			// snprintf(message, sizeof(message), "PID hijo: [%d]\n", getpid());
			// perror(message);
			// Caso pipes
			if (curr_cmd->input_fd != STDIN_FILENO)
			{
...
int execute_commands(t_mini *mini)
{
	...
	while (t_list_exec_cmd)
	{
		...
		if (!apply_redirections(curr_cmd))
		{
			t_list_exec_cmd = t_list_exec_cmd->next;
			continue ;
		}
		// Aplicar redirecciones antes de forking
		// apply_redirections(curr_cmd);

		//si hay mas comandos , se crea una pipe
		if (t_list_exec_cmd->next)
		{
			if (pipe(pipe_fd) == -1)
			{
		...
		else
		{
			if (curr_cmd->output_fd == -1)
				curr_cmd->output_fd = STDOUT_FILENO;
			//utlimo cmd escribe en terminal stdout
		}
		//idea es agregar la redirección al comando
		//entrada del comando
		curr_cmd->input_fd = input_fd;
